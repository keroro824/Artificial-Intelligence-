...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for ghost in range(len(livingGhostPositionDistributions)):
        allpositions.append(livingGhostPositionDistributions[ghost].argMax())

    choiceMinimumDistance = util.Counter()
    for action in legal:
        successorPosition = Actions.getSuccessor(pacmanPosition, action)        
        distancesToGhosts = []
        for pos in allpositions:
	        distancesToGhosts.append(self.distancer.getDistance(pos, successorPosition))
        distancesToGhosts.sort()
        try:
            choiceMinimumDistance[action] = -distancesToGhosts[0]
        except:
            print "Error TraceBack: Information report, status check: . . ."
            
            print distancesToGhosts

    return choiceMinimumDistance.argMax()
