...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for elem in self.legalPositions:
      ...
      if emissionModel[trueDistance] > 0: 
          allPossible[elem] = emissionModel[trueDistance] * self.beliefs[elem]
  ...
  def elapseTime(self, gameState):
    ...
    *** GORY DETAIL AHEAD ***
    ...
    for elem in self.legalPositions:
        distance = self.getPositionDistribution(self.setGhostPosition(gameState, elem))
        for d in distance:
            beliefs[d]+=distance[d] * self.beliefs[elem]
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    legalMove = self.legalPositions
    self.particles = util.Counter()
    i = 0
    while i!=self.numParticles:
        choose = random.choice(legalMove)
...
#        self.initializeUniformly(self.numParticles)
  ...
  def elapseTime(self, gameState):
    ...
    for elem in self.legalPositions:
        distance = self.getPositionDistribution(self.setGhostPosition(gameState, elem))
        for d,p in distance.items():
            beliefs[d]+=p* self.particles[elem]
    beliefs.normalize()
    self.particles = beliefs
  ...
  def initializeParticles(self):
    ...
    while i!=self.numParticles:
        ...
        while j!=self.numGhosts:
            choose = random.choice(self.legalPositions)
            GP.append(choose)
            j+=1
        newGP = tuple(GP)
        self.particles.append(newGP) 
        i+=1
  ...
  def observeState(self, gameState):
    ...
    ** Remember ** We store particles as tuples, but to edit a specific particle,
    ...
    for elem in self.particles:
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i]!=None:
                trueDistance = util.manhattanDistance(elem[i], pacmanPosition)
                weight *= emissionModels[i][trueDistance]
        allPossible[elem] += weight
    if allPossible.totalCount() == 0:
        ...
        newlist = []
        for elem in self.particles:
            new = list(util.sample(allPossible))
            newlist.append(tuple(new))
        self.particles = newlist
